...
export default function IncidentsTable({ onRunPlan }: { onRunPlan?: (id: string) => void }) {
    ...
    <div className="bg-white rounded-2xl shadow">
      <div className="px-4 py-3 border-b flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="h-9 rounded-lg border px-3 text-sm hover:bg-slate-50"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-slate-500">
              <th className="py-2 px-4">ID</th><th className="px-4">Region</th><th className="px-4">Type</th>
              <th className="px-4">Priority</th><th className="px-4">Status</th><th className="px-4">ETA</th>
              <th className="px-4">Updated</th><th className="px-4"></th>
            </tr>
          </thead>
          <tbody>
            ...
            {!loading && filtered.map((i, idx) => (
              <tr key={i.id} className={`border-t ${idx%2?"bg-slate-50/50":"bg-white"} hover:bg-blue-50/40 transition`}>
                ...
                <td className="px-4"><span className="px-2 py-1 rounded-lg text-xs bg-blue-50 text-blue-700 border border-blue-200">{i.type}</span></td>
                <td className="px-4">
                ...
                <td className="px-4">
                  <div className="flex gap-2 justify-end">
                    ...
                    <button className="text-sm rounded-lg bg-blue-600 text-white px-3 py-1.5 hover:bg-blue-700"
                            onClick={() => { console.log("Run Plan clicked for", i.id); onRunPlan?.(i.id); }}>
                      Run Plan
                    </button>
                  </div>
                </td>
            ...
            {!loading && filtered.length===0 && (
              <tr className="border-t"><td colSpan={8} className="px-4 py-6 text-center text-slate-500">No incidents match your search.</td></tr>
            )}
          </tbody>
        </table>
      </div>
