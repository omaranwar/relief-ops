...
export default function App() {
  ...
  const generatePlan = useCallback(async () => {
    ...
    try {
      ...
      const planText =
        resp?.plan ?? resp?.action_plan ?? (typeof resp === "string" ? resp : JSON.stringify(resp, null, 2));
  ...
  useEffect(() => {
    const start = () => {
      ...
      intervalRef.current = window.setInterval(() => setIncidentsKey(k => k + 1), autoRefreshMs);
    };
    const stop = () => {
      if (!intervalRef.current) return;
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key.toLowerCase() === "r" && !(e.metaKey || e.ctrlKey)) setIncidentsKey(k => k + 1);
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-100 dark:from-slate-900 dark:via-slate-900 dark:to-slate-900">
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <div>
          ...
          {selectedIncidentId && <p className="text-xs text-slate-500 mt-1">Selected Incident: <span className="font-medium">{selectedIncidentId}</span></p>}
        </div>

        <KpiCards selectedIncidentId={selectedIncidentId ?? undefined} refreshKey={incidentsKey} />
          ...
          <div className="col-span-12 xl:col-span-6">
            <div className="rounded-2xl bg-white/70 dark:bg-slate-800/60 backdrop-blur p-2 h-[420px] xl:h-[520px]">
              ...
              <div className="w-full h-full">
                <MapView key={`map-${selectedIncidentId ?? "none"}-${incidentsKey}`} incidentId={selectedIncidentId} refreshKey={incidentsKey} />
              </div>
            </div>
          </div>

          <div className="col-span-12 xl:col-span-6">
            <div id="action-plan" className="rounded-2xl bg-white/70 dark:bg-slate-800/60 backdrop-blur p-3 min-h-[420px] xl:min-h-[520px] max-h-[75vh] flex flex-col">
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-100">Action Plan</h2>
                <div className="flex items-center gap-2">
                  <button type="button" disabled={!selectedIncidentId || planLoading} onClick={generatePlan} className="rounded-md border border-slate-300 dark:border-slate-400 bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-50 px-3 py-1.5 text-xs font-semibold hover:bg-slate-200 dark:hover:bg-slate-600" title="Generate plan via Bedrock">
                    {planLoading ? "Generating…" : "Generate / Refresh Plan"}
                  </button>
                  <button type="button" disabled={!selectedIncidentId || planLoading} onClick={() => selectedIncidentId && fetchStoredPlan(selectedIncidentId)} className="rounded-md border border-slate-300 dark:border-slate-400 bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-50 px-3 py-1.5 text-xs font-semibold hover:bg-slate-200 dark:hover:bg-slate-600" title="Reload stored plan">
                    Reload Stored Plan
                  </button>
                </div>
              </div>
              ...
              {(planLoading || planError || planStatus) && (
                <div className="px-1 pb-1 text-xs">
                  ...
                  {!planLoading && planError && <span className="text-amber-500">No plan yet or failed to load: {planError}</span>}
                  {!planLoading && !planError && planStatus && <span className="text-slate-400">{planStatus}</span>}
                </div>
              )}
        ...
        <div className="rounded-2xl bg-white/70 dark:bg-slate-800/60 backdrop-blur p-2 mt-2">
          <IncidentsTable
            ...
            onView={(id) => {
              setSelectedIncidentId(id);
              setPlanKey(k => k + 1);
              setTimeout(() => document.getElementById("action-plan")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
            }}
            onRunPlan={async (id) => {
              setSelectedIncidentId(id);
              try {
                ...
                const planText = resp?.plan ?? resp?.action_plan ?? (typeof resp === "string" ? resp : JSON.stringify(resp, null, 2));
                setPlan(planText || null);
                setPlanStatus("Plan generated.");
              } catch (e: any) {
                setPlanError(e?.message ?? "Failed to generate plan");
        ...
        <footer className="text-xs text-slate-500 dark:text-slate-400 pt-4 border-t dark:border-slate-700">
          ...
          <button onClick={() => setIncidentsKey(k => k + 1)} className="ml-3 inline-flex items-center rounded-md border border-slate-300 dark:border-slate-500 bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-50 px-2 py-1 text-xs hover:bg-slate-200 dark:hover:bg-slate-600" title="Refresh incidents (R)">
            Refresh Incidents
          </button>
        </footer>
      </div>
